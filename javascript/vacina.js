window.onload = () => {
  ...
  const vacinaInput = document.getElementById("txtNovoNomeVacina");
  vacinaInput.addEventListener("keyup", () => {
    validarNome(vacinaInput);
  });
};

// Validador do nome da vacina (Ao menos 6 caracteres)
function validarNome(vacinaInput) {
  let content = vacinaInput.value;
  let labelElement = document.getElementById("log-nome");
  const regex = /^.{6,}$/;
...
function buscarVacinas() {
  ...
  const termoDePesquisa = document
    .querySelector('input[name="search-vacina"]')
    .value.toLowerCase();
...
function editarVacina(card_vacina) {
  localStorage.setItem(
    "indexVacina",
    JSON.stringify(card_vacina.dataset.index)
  );
...
function cadastrarVacina() {
  ...
  let doseSelecionada = document.querySelector(
    'input[name="dose"]:checked'
  ).value;
