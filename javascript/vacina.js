...
window.onload = () => {
    ...
    const search_vacina = document.getElementById("search_vacina");
    search_vacina.addEventListener("keyup", () => {
...
function construirGridVacinas() {    
    ...
    // Limpa o conteúdo da section antes de reconstruir
    section_grid.innerHTML = '';

    const searchTerm = document.getElementById('search_vacina').value.toLowerCase();

    for (let index = 0; index < vacinas.length; index++) {
        const nomeVacina = vacinas[index].nome.toLowerCase();
        
        if (nomeVacina.includes(searchTerm)) {
